package kyu4.Codewarsstyle_ranking_system;
...
Note: Codewars no longer uses this algorithm for its own ranking system. It uses a pure Math based solution that gives consistent results
no matter what order a set of ranked activities are completed at.
 */
public class User {
    ...
    public void incProgress(int taskLevel){
        ...
        if (taskLevel == 0 || taskLevel < -8 || taskLevel > 8){
            throw new IllegalArgumentException();
        }
        int currentRank = RANKS.indexOf(rank);
        int taskRank = RANKS.indexOf(taskLevel);
        incRank(pointsRewarded(currentRank, taskRank));
        System.out.println(rank+ " " + progress);
    }
    private int pointsRewarded(int userRank, int taskRank){
        int rankDifference = userRank - taskRank;
        if (rankDifference == 1){
            return 1;
        }
        if (rankDifference == 0){
    ...
    private void incRank(int points){
        if (RANKS.indexOf(rank) == RANKS.size()-1){
            return;
        }
        int tempPoints = progress + points;
        if (tempPoints < 100){
            this.progress = tempPoints;
            return;
        }
        int rankIncrease = tempPoints/100;
        int remainingProgress = tempPoints%100;
        if (RANKS.indexOf(rank)+rankIncrease >= 15){
            rank = 8;
            progress = 0;
            return;
        }
        rank = RANKS.get(RANKS.indexOf(rank)+rankIncrease);
